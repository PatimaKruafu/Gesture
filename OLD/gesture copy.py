...
def main():
    ...
    with HandLandmarker.create_from_options(hand_options) as landmarker, \
        ...
        frame_skip_counter = 0 # For processing every Nth frame
        PROCESS_EVERY_N_FRAMES = 1 # Set to 1 to process every frame, >1 to skip

        while cap.isOpened():
            ret, frame_bgr_original = cap.read() # Renamed to avoid confusion
            if not ret:
            ...
            # Start with the current camera frame (flipped)
            ...
            # latest_annotated_frame_rgb is updated by the callback
            if latest_annotated_frame_rgb is not None:
                # Ensure latest_annotated_frame_rgb is also flipped if not already done in callback logic
                # Assuming it's based on the already flipped input mp_image
                display_frame_bgr = cv2.cvtColor(latest_annotated_frame_rgb, cv2.COLOR_RGB2BGR)
            
            # --- HCI Logic (Operates on latest_hand_landmarks_data and latest_gestures_data) ---
            # (Your existing HCI logic for cursor, click, zoom goes here)
            # ... (Make sure this logic also uses display_frame_bgr for drawing feedback if needed) ...
            ...
            if latest_hand_landmarks_data:
                for handedness, landmarks in latest_hand_landmarks_data:
                    if handedness.lower() == "right":
                        ...
                        break
                if not active_hand_landmarks and latest_hand_landmarks_data:
            ...
            if active_hand_landmarks:
                ...
                if pinch_dist < PINCH_THRESHOLD:
                    if not is_pinching_for_click:
                        ...
                        # Draw click feedback on display_frame_bgr
            ...
            current_time_for_zoom = time.time() # Renamed to avoid conflict
            ...
            # Calculate FPS based on the time difference between processed frames
            # This will be more accurate if PROCESS_EVERY_N_FRAMES > 1
            # For simplicity, if PROCESS_EVERY_N_FRAMES = 1, this is just frame display rate
            if (new_frame_time - prev_frame_time) > 0: # Avoid division by zero
                fps = 1 / (new_frame_time - prev_frame_time)
            else:
                fps = 0 # Or some high value if frames are too fast
            prev_frame_time = new_frame_time
            
            # Now display_frame_bgr is guaranteed to be initialized
...
if __name__ == '__main__':
    ...
    main() # Call your main application function
    
    profiler.disable()
    stats = pstats.Stats(profiler).sort_stats('cumtime') # Sort by cumulative time
    stats.print_stats(30) # Print top 30 culprits