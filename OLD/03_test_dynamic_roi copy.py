...
#ROI_HALF_WIDTH = 0.10   # Smaller ROI: 20% of camera width
#ROI_HALF_HEIGHT = 0.15
    ...
    with HandLandmarker.create_from_options(hand_options) as landmarker:
        ...
        while cap.isOpened():
            ...
            if active_hand_landmarks:
                cursor_anchor_point = get_landmark(active_hand_landmarks, WRIST)
                cursor_anchor_point = get_landmark(active_hand_landmarks, WRIST)
                ...
                if last_wrist_pos_for_roi_check is not None:
                    ...
                    #temp_palm_anchor = mp.tasks.vision.NormalizedLandmark(x=cursor_anchor_point.x, y=cursor_anchor_point.y, z=0)
                    #dist_moved = calculate_distance(temp_palm_anchor, temp_last_wrist)
                    dist_moved = calculate_distance(cursor_anchor_point, last_wrist_pos_for_roi_check)
                    ...
                    if dist_moved < ROI_RECENTER_MOVEMENT_THRESHOLD:
                        if time_wrist_stable_at_pos_start_ms == 0: # Just became stable
                            ...
                            #last_wrist_pos_for_roi_check = mp.tasks.vision.NormalizedLandmark(x=cursor_anchor_point.x, y=cursor_anchor_point.y, z=getattr(cursor_anchor_point, 'z', 0))
                            last_wrist_pos_for_roi_check = cursor_anchor_point
            ...
            if active_hand_landmarks and can_attempt_cursor_control:
                # cursor_anchor_point is already defined (WRIST)
            ...
            if active_hand_landmarks and is_mouse_control_active and is_dragon_claw_active_current_frame:
            ...
            elif not active_hand_landmarks or not is_dragon_claw_active_current_frame : # Ensure mouse buttons are released if hand lost or gesture stops
                if is_left_pinching_for_drag:
                    pyautogui.mouseUp(button='left'); is_left_pinching_for_drag = False
                    print("Auto Left MouseUp (Hand Lost/Claw Released)")
